...
import type { Action } from "@/types/ActionRelated";

// ✅ Utility: safely get "MM.YYYY" from date string
...
export default function BmDashboard() {
  ...
  const months = useMemo(() => {
    ...
    for (const a of actions) {
      const key = monthKey(a.actionSince);
  ...
  const filtered = useMemo(() => {
    return actions.filter((a) => {
      ...
      const monthMatch =
        filterMonth === "All" || monthKey(a.actionSince) === filterMonth;
      const statusMatch =
  ...
  const groupedByDate = useMemo(() => {
    ...
    for (const a of filtered) {
      const dayKey = new Date(a.actionSince).toISOString().split("T")[0]; // YYYY-MM-DD
